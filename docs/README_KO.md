...
| WinUI 뷰어 | `SkyrimDiagWinUI/SkyrimDiagDumpToolWinUI.exe` | 찾기 쉬운 대화형 뷰어 실행 런처 |
| WinUI 앱 런타임 | `SkyrimDiagWinUI/app/` | self-contained 뷰어 앱, 런타임 파일, 네이티브 분석 엔진, data |
...
3. `SkyrimDiagDumpToolWinUI.exe`를 실행한 뒤 `.dmp`를 선택하고 "지금 분석"
   - 또는 커맨드라인에서 `SkyrimDiagDumpToolWinUI.exe "경로\\dump.dmp"` 실행
   - `v0.2.53+`에서는 `SkyrimDiagWinUI` 최상위의 `SkyrimDiagDumpToolWinUI.exe`가 런처입니다. 많은 런타임 파일은 `SkyrimDiagWinUI\\app` 아래에 모아 둡니다.
...
- 기본 (`OutputDir=`를 비워둘 때): MO2 `overwrite\SKSE\Plugins\Tullius Ctd Logs\`
- 변경: `SkyrimDiagHelper.ini`에서 `OutputDir=` 설정
  비워 두면 기본 `Tullius Ctd Logs` 폴더를 사용합니다. 직접 지정할 때는 따옴표 없이 경로 문자열만 적으면 됩니다. 절대경로를 써도 되고, 상대경로는 헬퍼 폴더 기준으로 해석됩니다.
